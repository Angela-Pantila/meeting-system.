...
-- Profiles: everyone can read; users edit self; admins edit anyone.
...
create policy "rooms select" on public.meeting_rooms for select using (auth.role() = 'authenticated');
create policy "rooms insert" on public.meeting_rooms for insert with check (auth.role() = 'authenticated');
create policy "rooms update" on public.meeting_rooms for update using (auth.role() = 'authenticated');
create policy "rooms delete" on public.meeting_rooms for delete using (auth.role() = 'authenticated');
...
create policy "meeting docs read" on storage.objects
  for select using (bucket_id = 'meeting-docs' and auth.role() = 'authenticated');

create policy "meeting docs upload" on storage.objects
  for insert with check (bucket_id = 'meeting-docs' and auth.role() = 'authenticated');

create policy "meeting docs delete" on storage.objects
  for delete using (bucket_id = 'meeting-docs' and auth.role() = 'authenticated');
